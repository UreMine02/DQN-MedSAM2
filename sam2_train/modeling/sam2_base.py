...
class SAM2Base(torch.nn.Module):
    ...
    def _prepare_memory_conditioned_features(
        ...
        obj_ptr = torch.cat(to_cat_obj_ptr, dim=0)
        memory_pos_embed = torch.cat(to_cat_memory_pos_embed, dim=0)
        
        # # NOTE: TEST SEMANTIC FILTERING
        # obj_gating_score = obj_ptr.sum(dim=0, keepdim=True).sigmoid()
        # obj_ptr = obj_ptr * obj_gating_score
        # memory = torch.cat([memory, obj_ptr], dim=0)
