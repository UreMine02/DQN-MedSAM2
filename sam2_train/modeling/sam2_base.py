...
from sam2_train.modeling.backbones.updates import BackboneUpdates
...
class SAM2Base(torch.nn.Module):
    def __init__(
        ...
        # Loops of final iterative masking
        self.num_iterative_loop = num_iterative_loop
    ...
    def _forward_sam_heads(
        ...
        # print("Hi", backbone_features.size(), self.sam_image_embedding_size)
        ...
        for i in range(self.num_iterative_loop):
            (
                ...
                image_embeddings=backbone_features, # torch.Size([1, 256, 64, 64])
                image_pe=self.sam_prompt_encoder.get_dense_pe(), # torch.Size([1, 256, 64, 64])
                sparse_prompt_embeddings=sparse_embeddings, # torch.Size([1, 3, 256]
                dense_prompt_embeddings=dense_embeddings, # torch.Size([1, 256, 64, 64]
            ...
            #     backbone_features = self.backboneUpdate(backbone_features, low_res_multimasks)
                ...
                # low_res_token = attn(res_mask_encoding, backbone_features)
                # backbone_features = low_res_token

        low_res_multimasks = low_res_multimasks.float() # torch.Size([1, 1, 256, 256])
        ...
        if multimask_output:
            ...
            best_iou = ious[batch_inds, best_iou_inds].reshape(-1,1)
    ...
    def _prepare_memory_conditioned_features(
        ...
        if not is_init_cond_frame:
            ...
            for t_pos, prev in t_pos_and_prevs:
                ...
                feats = prev["maskmem_features"].cuda(device=device, non_blocking=True)
        ...
        if self.highres_gating == "by_ptr":
            high_res_features = []
            for highres, ptr2high_proj, high2high_proj, bias in zip(highres_vision_feats, self.ptr2high_gating_proj, self.high2high_gating_proj, self.highres_gating_bias):
    ...
    def track_step(
        ...
        if mask_inputs is not None and self.use_mask_input_as_output_without_sam:
            ...
            if self.highres_gating == "by_lowres":
                ...
                layers = zip(high_res_features, self.low2high_gating_proj, self.high2high_gating_proj, self.ptr2high_gating_proj, self.highres_gating_bias)
                for highres, low2high_proj, high2high_proj, ptr2high_proj, bias in layers:
